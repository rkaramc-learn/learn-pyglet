...
>
> All core mechanics and UI have been implemented in `game_running.py` and `mechanics/health.py`.
> Win/loss conditions work as specified. UI uses `HealthBar` component.
...
The following beads completed this feature:

- `pyglet-ciz.2`: Integrated `mechanics/health.py` into `game_running.py`
- `pyglet-ciz.3`: Replaced raw rectangles with `HealthBar` UI component
- `pyglet-ciz.4`: Replaced boolean `game_over` with `GameStateManager`

Future enhancements (tracked as ideas):

- `pyglet-rb4`: Mouse health regeneration mechanic
- `pyglet-jsd`: Kitten "Aggro/Sprint" mode
